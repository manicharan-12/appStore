...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, input: ''}

  getFilteredInputData = filteredData => {
    const {input} = this.state
    const filterInputData = filteredData.filter(eachData =>
      eachData.appName.toLowerCase().includes(input.toLowerCase()),
    )
  ...
  render() {
    ...
    const filterInputData = this.getFilteredInputData(filteredData)
    return (
      <div className="bg-container">
        <h1 className="main-heading">App Store</h1>
        <div className="input-container">
          <input type="search" placeholder="Search" onChange={this.inputVal} />
          <img
